...
const {
  defaults, pick, each, range, compact, first,
} = require('lodash');
...
const formatMovie = (movie) => {
  const details = pick(movie, ['title', 'release_date', 'popularity', 'vote_average', 'id', 'vote_count', 'poster_path']);
...
module.exports.getMovies = () => Promise.resolve(getMovies())
  .bind({
    movies: [],
  })
  .then((response) => {
    each(response.results, (result) => {
      this.push(result);
    });

    return range(response.page, response.total_pages);
  })
  .map(page => getMovies(page)
    .then(response => response.results), { concurrency: 1 })
  .each((results) => {
    each(results, (result) => {
  ...
  .then(() => {
    this.movies = compact(this.movies);
    return this.movies;
  })
...
module.exports.searchMovie = (title, year) => {
  ...
  return makeRequest('/search/movie', query)
    .then(results => first(results.results))
